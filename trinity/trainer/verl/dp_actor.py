...
class DataParallelPPOActor(DPActor):
    ...
    def update_policy(self, data: DataProto):  # noqa: C901
        ...
        # rollout_is_weights will be used in policy loss if provided by workflow.
        # rollout_log_probs is equal to old_log_prob in Trinity.
        # Some policy loss args are optional; keep only keys present in this batch.
        batch_keys = set(data.batch.keys())
        select_keys = list({key for key in select_keys if key in batch_keys})
