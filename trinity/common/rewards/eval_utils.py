...
def parse_with_timeout(pred: str, parsing_timeout: int = 5, **kwargs) -> list[str]:
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            # If parsing fails for any reason, return empty list or handle gracefully
            # This prevents crashes from invalid input that math_verify can't parse
            return []
...
def verify_with_timeout(gold: str, target: str, timeout_seconds: int = 5, **kwargs) -> bool:
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            # This prevents crashes from invalid input that math_verify can't handle
            return False
