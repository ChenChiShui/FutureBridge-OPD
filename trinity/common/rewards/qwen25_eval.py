...
def verify_math_answer(response_text: str, ground_truth: str) -> Tuple[float, Dict[str, Any]]:
    ...
    try:
        ...
        # If any parsing or comparison error occurs, treat as incorrect
        return 0.0, {
...
def extract_answer(response_text: str) -> Optional[str]:
    ...
    # Extract 'answer is ...'
    search_patterns = [r"(?:final|the)\s+answer\s+is\s*:?\s*(.+)", r"###\s*:?\s*(.+)"]
...
def _parse_symbolic(s: str) -> Any:
    ...
    try:
        ...
        except (sympy.SympifyError, TypeError, SyntaxError, AttributeError, ValueError):
...
def _check_numeric_value_equality(a_sym: Any, b_sym: Any) -> Optional[bool]:
    ...
    try:
        # Check if both are strings (parsing failed), compare directly
        if isinstance(a_sym, str) and isinstance(b_sym, str):
            return None  # Let other checks handle string comparison
...
def symbolic_equal(a: str, b: str) -> bool:
    ...
    try:
        ...
        # If any error occurs during symbolic comparison, fall back to string comparison
        return str(a) == str(b)
