...
class GuidedOPDAlfworldWorkflow(Workflow):
    ...
    # Custom field to pass is_expert from experience.info to batch tensor
    ...
    def __init__(
        ...
        *,
        ...
        # Cross-tokenizer shared-support OPD (lazy init, same as OPD workflow)
    ...
    async def _run_episode(self, env) -> List[Experience]:
        ...
        turn_messages: List[List[dict]] = []  # Messages sent to model per turn
        teacher_turn_flags: List[bool] = []  # Track which turns were teacher

        kwargs = {**self.rollout_args, "n": 1}
        if kwargs.get("logprobs") is None:
            kwargs["logprobs"] = 0

        # Compute beta from training step (batch_id), not run_id_base
        if self.is_eval:
            beta = 0.0
        else:
            global_step = 0
            ...
            if bid is not None:
                print(f"[GuidedOPD-DEBUG] task.batch_id={bid!r} type={type(bid)}", flush=True)
        ...
        for r in range(self.max_env_steps):
            ...
            if use_teacher:
                ...
                # (teacher's text, but student's tokenization)
                ...
                try:
                    ...
                    # Fallback: skip this turn, execute parsed action
                ...
                # Mark as expert (teacher turn → SFT loss)
                ...
                # Execute teacher's action in environment
                action = parse_action(response_text)
            else:
                # Student generates response (same as OPD workflow)
                ...
                # Mark as non-expert (student turn → OPD loss)
                ...
                # Execute student's action in environment
                action = parse_action(response_text)

            # Shared environment history (regardless of who acted)
            memory.append({"role": "assistant", "content": response_text})
            history.append(_format_history(format_obs, r + 1, action))
            observation, reward, done, info = env.step(action)
        ...
        for i, exp in enumerate(turn_experiences):
            ...
            # Student turn: compute teacher logprobs via cross-tokenizer OPD
            messages_i = turn_messages[i]

            # Defensive: skip if messages_i is empty (would raise IndexError
            # inside apply_chat_template). Fall back to zero teacher_logprobs.
            if not messages_i:
            ...
            # Get teacher logprobs on teacher's own token sequence
            teacher_logprobs = await self.teacher_model.logprobs_async(
                tokens=teacher_tokens,
                temperature=self.temperature,
            )

            # Shared-support OPD: byte-align student and teacher response tokens
            from trinity.common.workflows.envs.TCOD.alfworld.cross_tokenizer_opd import (
        ...
        # Set reward and trajectory-level metrics for all experiences
