...
class GuidedOPDScienceworldWorkflow(Workflow):
    ...
    async def _run_episode(self, env) -> List[Experience]:
        ...
        for r in range(self.max_env_steps):
            ...
            if use_teacher:
                # Teacher generates response (same tokenizer family → reuse student chat format)
        ...
        # Same tokenizer family → directly compute teacher logprobs on student tokens
        ...
        for i, exp in enumerate(turn_experiences):
            ...
            # Student turn: compute teacher logprobs on student's tokens
            # (reuse OPD_workflow.py approach — same tokenizer, no cross-tokenizer)
