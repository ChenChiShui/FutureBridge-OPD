"""TCOD Backward-to-Forward workflow for ScienceWorld."""
...
@WORKFLOWS.register_module("TCOD_b2f_scienceworld_workflow")
class TCOD_b2f_scienceworld_workflow(Workflow):
    """TCOD Backward-to-Forward workflow for ScienceWorld."""
    ...
    def _linear_checkpoint_step(
        ...
        checkpoint_step = max(
            0, min(max_expert_actions - reduction, max_expert_actions)
        )
    ...
    async def run_async(self) -> List[Experience]:
        ...
        probe_env = _create_scienceworld_env(
            self.task_desc,
            max_env_steps=self.max_env_steps,
            generate_gold_path=True,
        )
        try:
            self._expert_actions = probe_env.get_gold_action_sequence()
        finally:
            probe_env.close()

        effective_checkpoint_step = 0
        if self.checkpoint_strategy == "linear":
            effective_checkpoint_step = self._linear_checkpoint_step(self._expert_actions)

        if effective_checkpoint_step > 0:
            (
                env,
                observation,
                info,
            ...
            try:
                if replay_done:
                    ...
                    self._final_reward = checkpoint_reward
                    return []
    ...
    async def _run_episode(self, env) -> List[Experience]:
        ...
        for r in range(self.max_env_steps):
            ...
            if response.logprobs is None:
                raise RuntimeError(
                    "TCOD_b2f_scienceworld_workflow requires student model to return "
        ...
        return await self._finalize_turn_responses(
            turn_responses,
            start_step=0,
    ...
    async def _run_episode_from_checkpoint(
        ...
        turn_responses: List[Experience] = []
        memory_snapshots: List[List[dict]] = []
        best_score = max(int(round(checkpoint_reward * 100)), info.get("score", 0))
    ...
    async def _finalize_turn_responses(
        ...
        *,
        ...
        total_expert_actions = 0 if self.is_eval else len(self._expert_actions)
