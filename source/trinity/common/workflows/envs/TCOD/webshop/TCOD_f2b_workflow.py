"""TCOD Forward-to-Backward workflow for WebShop."""
...
@WORKFLOWS.register_module("TCOD_f2b_webshop_workflow")
class TCOD_f2b_webshop_workflow(Workflow):
    """TCOD Forward-to-Backward workflow for WebShop task."""
    ...
    async def _run_episode(self, env, session_id: int) -> List[Experience]:
        ...
        for r in range(effective_steps):
            ...
            if response.logprobs is None:
                raise RuntimeError(
                    "TCOD_f2b_webshop_workflow requires student model to return logprobs. "
