"""TCOD Backward-to-Forward workflow for AlfWorld."""
...
class TCOD_b2f_alfworld_workflow(Workflow):
    """TCOD Backward-to-Forward workflow for AlfWorld task."""
    ...
    def _linear_checkpoint_step(self, predefined_actions: Optional[List[str]] = None) -> Optional[int]:
        """Compute checkpoint step using linear decay strategy."""
    ...
    async def _run_episode_from_checkpoint(
        ...
        trajectory_kl_divergence = sum(per_turn_kl_sums)
        total_expert_actions = len(self.raw_task.get("actions", []))
        expert_actions_remaining = total_expert_actions - start_step
