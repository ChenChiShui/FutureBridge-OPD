...
NOTE: You need to install the alfworld dataset in first: 
"""
import argparse
import glob
import json
...
def create_dataset_files(game_data_path, output_dir, train_size=None, test_size=None):
    ...
    # test_unseen: ALL games from valid_unseen (the 134-task unseen test set
    # used for the evaluations reported in the paper)
    unseen_game_files = sorted(
    ...
    test_unseen_data = [
        {"game_file": game_file_path, "target": ""} for game_file_path in unseen_game_files
    ]
    if test_unseen_data:
        dataset_dict["test_unseen"] = test_unseen_data
    else:
        print("WARNING: no valid_unseen game files found; test_unseen.jsonl not written.")
    ...
    print(
        f"Created dataset with {len(train_data)} train, {len(test_data)} test "
        f"and {len(test_unseen_data)} test_unseen examples."
    )
