...
class TestLauncherMain(unittest.TestCase):
    ...
    def test_main_run_in_dlc(self, mock_init, mock_load, mock_both, mock_setup, mock_stop):
        ...
        with mock.patch.dict(
            ...
            with mock.patch(
                "argparse.ArgumentParser.parse_args",
                return_value=mock.Mock(
                    command="run", config="dummy.yaml", dlc=True, plugin_dir="<plugins>"
                ),
            ):
                launcher.main()
            ...
            mock_init.assert_called_once_with(
                ...
                runtime_env={
                    "env_vars": {
                        launcher.PLUGIN_DIRS_ENV_VAR: "<plugins>",
    ...
    def test_multi_stage_run(
        ...
        mock_load.return_value = config
        mock_checkpoint_path.return_value = "<checkpoint>"
        with mock.patch.dict(
            launcher.MODE_MAP,
            {
                "train": mock_train,
            ...
            with mock.patch(
                "argparse.ArgumentParser.parse_args",
                return_value=mock.Mock(
                    command="run", config="dummy.yaml", dlc=False, plugin_dir="<plugins>"
                ),
            ):
                launcher.main()
            ...
            self.assertEqual(mock_both.call_args[0][0].model.model_path, "<checkpoint>")
            self.assertEqual(
